...
fun DailyEntryScreen(viewModel: DailyEntryViewModel, initialDate: String) {
    ...
    val visibleItemIds = remember(uiState.selectedDate, uiState.taskItems) {
        val defaultVisibleIds = if (uiState.hasExistingRecord) {
            uiState.taskItems.filter(TaskItemInputState::hasExistingValue).map(TaskItemInputState::taskItemMasterId).toMutableSet()
        } else {
            uiState.taskItems.map(TaskItemInputState::taskItemMasterId).toMutableSet()
        }
    ...
    LazyColumn(
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Text(
                    text = if (uiState.hasExistingRecord) "기존에 저장된 기록을 먼저 불러왔습니다." else "새 기록을 작성할 수 있습니다.",
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                ...
                Button(onClick = openDatePicker, modifier = Modifier.align(Alignment.CenterVertically)) { Text("달력") }
        ...
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            when (item.valueType) {
                ...
                ValueType.BOOLEAN -> Row(
                    ...
                    Text("완료 여부")
                    Switch(checked = item.booleanValue, onCheckedChange = { checked -> onItemChanged(item.copy(booleanValue = checked, checked = checked)) })
                }
